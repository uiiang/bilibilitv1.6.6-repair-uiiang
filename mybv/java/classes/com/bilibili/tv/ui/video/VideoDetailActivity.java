...
public final class VideoDetailActivity extends BaseActivity
    ...
    private EntryType mEntryType = EntryType.UGC_BY_AVID;
    private i A;
    private boolean B;
    private ImageView b;
    private TextView cc;
    private LinearLayout uperContainer;
    ...
    private e v;
    private com.bilibili.tv.ui.video.widget.VideoCardAdapter relateCardAdapter;
    private com.bilibili.tv.ui.video.widget.BiliVideoDetailBinder relateBinder;
    private add w;
    private g x;
    private boolean y;
    private boolean z;
    ...
    private com.bilibili.tv.ui.video.widget.VideoCardAdapter episodesCardAdapter;
    // 多合集列表容器
    public LinearLayout seasonsContainer;
    // 存储动态创建的合集列表
    private List<SeasonSectionView> seasonSectionViews = new ArrayList<>();
    private List<com.bilibili.tv.ui.video.widget.VideoListSection> ugcSeasonSections = new ArrayList<>();
    private com.bilibili.tv.ui.video.widget.VideoListNavigator videoListNavigator;
    ...
    public static class SeasonSectionView {
        ...
        public SeasonSectionView(TextView titleView, RecyclerView recyclerView, int sectionId) {
            this.titleView = titleView;
            this.recyclerView = recyclerView;
            this.sectionId = sectionId;
        }
    ...
    private final void k() {
        ...
        if (this.r != null) {
            ...
            this.r.setAdapter(relateCardAdapter);
            relateCardAdapter.setOnItemClickListener(new com.bilibili.tv.ui.video.widget.VideoCardAdapter.OnItemClickListener() {
                @Override
        ...
        this.r.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (hasFocus) {
                    ...
                    for (int ci = 0; ci < VideoDetailActivity.this.r.getChildCount(); ci++) {
                        View child = VideoDetailActivity.this.r.getChildAt(ci);
                        if (child != null && child.hasFocus()) {
                            relateVideoFocusPosition = ci;
                            break;
                        }
                    }
                }
        ...
        if (this.episodes_video != null) {
            ...
            episodesCardAdapter.setOnItemClickListener(new com.bilibili.tv.ui.video.widget.VideoCardAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(Object data, int position) {
                    episodesVideoFocusPosition = position;
                }
            });
    ...
    private com.bilibili.tv.ui.video.widget.VideoListSection findVideoListSection(int sectionId) {
        for (com.bilibili.tv.ui.video.widget.VideoListSection vls : ugcSeasonSections) {
            if (vls.getSectionId() == sectionId) {
                // Log.d(C, "findVideoListSection | 找到section | sectionId=" + sectionId);
                return vls;
            }
        }
        // Log.w(C, "findVideoListSection | 未找到section | sectionId=" + sectionId
        //         + " | ugcSeasonSections.size=" + ugcSeasonSections.size());
        return null;
    }

    private int getDataSizeForSection(int sectionId) {
        for (com.bilibili.tv.ui.video.widget.VideoListSection vls : ugcSeasonSections) {
            if (vls.getSectionId() == sectionId) {
                return vls.getDataSize();
            }
        }
        return 0;
    }
    ...
    private final void m() {
        // Log.i(C, "========== VideoDetailActivity INIT | CODE_VERSION=" + CODE_VERSION + " ==========");
        ...
        if (!TextUtils.isEmpty(this.mSeasonId)) {
            ...
            mIsPgcMode = true;
            // Log.i("VideoDetailActivity", "m() EntryType=PGC_BY_SEASON_ID, seasonId=" + mSeasonId);
        } else if (this.s > 0) {
            mEntryType = EntryType.UGC_BY_AVID;
            mIsPgcMode = false;
            // Log.i("VideoDetailActivity", "m() EntryType=UGC_BY_AVID, avid=" + s);
        } else {
    ...
    public boolean dispatchKeyEvent(KeyEvent keyEvent) {
        if (this.w == null || relateCardAdapter == null) {
            return super.dispatchKeyEvent(keyEvent);
        }
        Integer valueOf = keyEvent != null ? Integer.valueOf(keyEvent.getAction()) : null;
        Integer valueOf2 = keyEvent != null ? Integer.valueOf(keyEvent.getKeyCode()) : null;
        
        String actionName = (valueOf != null && valueOf.intValue() == 0) ? "DOWN" : (valueOf != null && valueOf.intValue() == 1) ? "UP" : "UNKNOWN";
        String keyName = getKeyName(valueOf2 != null ? valueOf2.intValue() : 0);
        ...
        if (direction == KeyEvent.KEYCODE_DPAD_DOWN) {
            ...
            if (currentListType == LIST_TYPE_EP_LAYOUT) {
                ...
                if (currentSectionIndex > 0) {
                    ...
                    savedPosition = getSeasonSectionFocusPosition(prevSection.sectionId);
                } else {
                    // 移动到分P列表或播放按钮
    ...
    private void refreshVisualOrder() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof com.bilibili.tv.ui.video.widget.VideoListSection) {
                com.bilibili.tv.ui.video.widget.VideoListSection section =
                ...
                for (SeasonSectionView ssv : seasonSectionViews) {
                    if (ssv.sectionId == sectionId && !visualSorted.contains(ssv)) {
                        visualSorted.add(ssv);
                        break;
                    }
                }
            }
    ...
    private String getSeasonSectionIdsString() {
        ...
        for (int i = 0; i < seasonSectionViews.size(); i++) {
            if (i > 0) sb.append(", ");
            sb.append(seasonSectionViews.get(i).sectionId);
        }
        sb.append("]");
        return sb.toString();
    }

    private com.bilibili.tv.ui.video.widget.VideoListSection findSectionByRecyclerView(RecyclerView rv) {
        for (SeasonSectionView ssv : seasonSectionViews) {
            if (ssv.recyclerView == rv) {
                return findVideoListSection(ssv.sectionId);
            }
        }
        return null;
    }
            ...
            if (recyclerViewId == R.id.season_section_recycler) {
                for (SeasonSectionView ssv : seasonSectionViews) {
                    if (ssv.recyclerView == recyclerView) {
                        com.bilibili.tv.ui.video.widget.VideoListSection targetVls = null;
                        for (com.bilibili.tv.ui.video.widget.VideoListSection vls : ugcSeasonSections) {
                            if (vls.getSectionId() == ssv.sectionId) {
                                targetVls = vls;
                                break;
                            }
                        }
                        ...
                        int dataPosition = -1;
                        if (targetVls != null) {
                            dataPosition = targetVls.getDataPositionForView(currentFocus);
                        }
    ...
    private int getVisibleIndex(RecyclerView rv, View view) {
        if (rv == null || view == null) return -1;
        for (int i = 0; i < rv.getChildCount(); i++) {
            if (rv.getChildAt(i) == view) return i;
        }
        return -1;
    }
    ...
    private final void loadUgcByAvid() {
        ...
        Log.i("VideoDetailApi", "========== UGC_BY_AVID Request ==========");
    ...
    private interface PgcLoadCallback {
        void onSuccess(PgcInfo pgcInfo);
        void onError(String message);
    }

    private void loadPgcSeasonInfo(String seasonId, final PgcLoadCallback callback) {
        final String accessKey = mg.a(this).e();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    params.put("track_path", "0");
                    params.put("platform", "android");
                    params.put("mobi_app", com.bilibili.api.BiliConfig.e());
                    params.put("appkey", com.bilibili.api.BiliConfig.a());
                    params.put("build", String.valueOf(com.bilibili.api.BiliConfig.c()));
                    
                    com.bilibili.nativelibrary.SignedQuery signedQuery = com.bilibili.nativelibrary.LibBili.a(params);
                    String signedUrl = "http://api.bilibili.com/pgc/view/app/season?" + signedQuery.toString();
                    
                    Log.i("PgcSeasonApi", "Request URL: " + signedUrl);
                    
                    retrofit2.Call<JSONObject> call = ((MyBiliApiService) vo.a(MyBiliApiService.class)).getPgcSeasonInfo(signedUrl);
                    ...
                    if (code != 0) {
                        callback.onError("API error: " + jsonObj.getString("message"));
                        return;
                    }
                    ...
                    Log.e("PgcSeasonApi", "loadPgcSeasonInfo error: " + e.getMessage(), e);
    ...
    private BiliVideoDetail convertPgcToBiliVideoDetail(PgcInfo pgcInfo) {
        ...
        detail.mBangumiInfo = new BiliVideoDetail.BangumiInfo();
        detail.mBangumiInfo.mSeasonId = String.valueOf(pgcInfo.seasonId);
        detail.mBangumiInfo.mTitle = pgcInfo.title;
        ...
        if (pgcInfo.producer != null && pgcInfo.producer.list != null && !pgcInfo.producer.list.isEmpty()) {
            ...
            detail.mOwner.name = "";
            detail.mOwner.face = "";
            Log.i("PgcInfo", "No producer or up_info, hiding staff area");
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    android.util.Log.e("HistoryApi", "loadHistory error: " + e.getMessage());
                }
    ...
    private final void fallbackLoadHistory(BiliVideoDetail biliVideoDetail, String accessKey) {
        ...
        ((VideoApiService) vo.a(VideoApiService.class)).getVideoDetails(
            ...
            @Override
            public void onError(Throwable th) {
                Log.e("HistoryApi", "Fallback error: " + th.getMessage());
            }
            @Override
                ...
                if (result != null && result.data != null) {
                    ...
                    if (data.containsKey("history")) {
                        ...
                        if (historyObj != null) {
                            ...
                            Log.i("HistoryApi", "Fallback history: cid=" + history.mCid + ", progress=" + history.mProgress + "s");
                            biliVideoDetail.mHistory = history;
                        }
                    } else {
                        Log.i("HistoryApi", "Fallback: no history data found");
                    }
                }
                updateHistoryDisplay(biliVideoDetail);
                if (VideoDetailActivity.this.historyPlayBtnLayout != null &&
    ...
    private void updateBiliVideoDetailWithPgcInfo(BiliVideoDetail biliVideoDetail, PgcInfo pgcInfo) {
        ...
        if (biliVideoDetail.mBangumiInfo == null) {
            biliVideoDetail.mBangumiInfo = new BiliVideoDetail.BangumiInfo();
        }
        ...
        if (biliVideoDetail.mPageList == null) {
            biliVideoDetail.mPageList = new ArrayList<>();
        }
        biliVideoDetail.mPageList.clear();
        for (int i = 0; i < pgcInfo.episodes.size(); i++) {
            PgcInfo.Episode ep = pgcInfo.episodes.get(i);
            BiliVideoDetail.Page page = convertEpisodeToPage(ep, i + 1);
    ...
    private BiliVideoDetail.Page convertEpisodeToPage(PgcInfo.Episode episode, int pageIndex) {
        ...
        page.mEpisodeId = episode.id;
        page.mSeasonId = String.valueOf(mPgcInfo != null ? mPgcInfo.seasonId : 0);
        page.mEpCover = episode.cover;
        page.mBvid = episode.bvid;
        
        if (mPgcInfo != null) {
            int pgcType = mPgcInfo.type;
            if (pgcType == 2) {
                page.mFrom = "movie";
            } else {
                page.mFrom = "bangumi";
    ...
    private void showPgcEpisodes(PgcInfo pgcInfo) {
        ...
        createPgcEpisodesSectionView(pgcInfo);
    }

    private void createPgcEpisodesSectionView(PgcInfo pgcInfo) {
        if (seasonsContainer == null || pgcInfo == null || pgcInfo.episodes == null) {
            return;
        }

        if (seasonsContainer.getVisibility() != View.VISIBLE) {
        ...
        for (int i = 0; i < pgcInfo.episodes.size(); i++) {
            ...
            JSONObject statForUnity = episode.statForUnity;
            if (statForUnity != null && statForUnity.containsKey("vt")) {
                JSONObject vtObj = statForUnity.getJSONObject("vt");
                if (vtObj != null) {
                    String pureText = vtObj.getString("pure_text");
                    if (!TextUtils.isEmpty(pureText)) {
                        detail.vtText = pureText;
                    } else {
        ...
        com.bilibili.tv.ui.video.widget.PageVideoDetailBinder binder = new com.bilibili.tv.ui.video.widget.PageVideoDetailBinder(true);
        ...
        RecyclerView sectionRecyclerView = listSection.getRecyclerView();
        if (sectionRecyclerView != null) {
            sectionRecyclerView.setNextFocusUpId(R.id.video_detail_coin);
        }

        SeasonSectionView seasonSectionView = new SeasonSectionView(listSection.getTitleView(), sectionRecyclerView, sectionId);
        seasonSectionViews.add(seasonSectionView);
        ugcSeasonSections.add(listSection);
        if (videoListNavigator != null) {
            videoListNavigator.registerSection(listSection);
        }
    ...
    private void showPgcSections(PgcInfo pgcInfo) {
        ...
        seasonSectionViews.clear();
        if (videoListNavigator != null) {
            videoListNavigator.clearSections();
        }
    ...
    private void createSeasonsSectionView(List<PgcInfo.Season> seasons, int sectionIndex) {
        ...
        com.bilibili.tv.ui.video.widget.PgcSeasonBinder binder = new com.bilibili.tv.ui.video.widget.PgcSeasonBinder();
        listSection.setData(list, binder);
    ...
    private void createPgcSectionView(PgcInfo.Section section, int sectionIndex) {
        ...
        com.bilibili.tv.ui.video.widget.BiliVideoDetailBinder binder = new com.bilibili.tv.ui.video.widget.BiliVideoDetailBinder();
        listSection.setData(list, binder);
    ...
    /* compiled from: BL */
    /* loaded from: classes.dex */
    public static final class f extends adc.a implements View.OnClickListener {
        public static final a Companion = new a(null);
        ...
        public ScalableImageView getCoverImageView() { return this.n; }
        public TextView getTitleView() { return this.o; }
        public TextView getUpView() { return this.p; }
        public TextView getPlayCountView() { return this.q; }
        public TextView getPubdateView() { return this.r; }
        public TextView getDurationView() { return this.duration; }
        public TextView getDanmakuInImageView() { return this.danmakuInImage; }
        public DrawRelativeLayout getRootLayout() { return this.s; }
    ...
    final class i extends vn<BiliVideoDetail> {
        ...
        private void addStaffView(LinearLayout container, String name, long mid, String faceUrl, int index, int totalCount) {
            DrawLinearLayout wrapper = new DrawLinearLayout(VideoDetailActivity.this);
            ...
            int contentPaddingH = VideoDetailActivity.this.getResources().getDimensionPixelSize(R.dimen.px_12);
            int contentPaddingV = VideoDetailActivity.this.getResources().getDimensionPixelSize(R.dimen.px_8);
            wrapper.setPadding(contentPaddingH, contentPaddingV, contentPaddingH, contentPaddingV);
            
            com.bilibili.tv.widget.CircleImageView avatarView = new com.bilibili.tv.widget.CircleImageView(VideoDetailActivity.this);
            int avatarSize = VideoDetailActivity.this.getResources().getDimensionPixelSize(R.dimen.px_32);
            LinearLayout.LayoutParams avatarLp = new LinearLayout.LayoutParams(avatarSize, avatarSize);
            int avatarMargin = VideoDetailActivity.this.getResources().getDimensionPixelSize(R.dimen.px_6);
            ...
            staffView.setTextSize(TypedValue.COMPLEX_UNIT_PX, VideoDetailActivity.this.getResources().getDimension(R.dimen.px_28));
            ...
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
            int margin = VideoDetailActivity.this.getResources().getDimensionPixelSize(R.dimen.px_6);
        ...
        private void createSeasonSectionView(BiliVideoDetail.SectionInfo sectionInfo, int sectionIndex, int totalSections, BiliVideoDetail biliVideoDetail) {
            ...
            RecyclerView sectionRecyclerView = section.getRecyclerView();
            if (sectionRecyclerView != null) {
                sectionRecyclerView.setNextFocusUpId(R.id.video_detail_coin);
            }

            SeasonSectionView seasonSectionView = new SeasonSectionView(section.getTitleView(), sectionRecyclerView, sectionId);
            VideoDetailActivity.this.seasonSectionViews.add(seasonSectionView);
            ugcSeasonSections.add(section);
            if (videoListNavigator != null) {
                videoListNavigator.registerSection(section);
            }
        ...
        private final void d(BiliVideoDetail biliVideoDetail) {
            ...
            createUgcEpisodesSectionView(biliVideoDetail);
        }

        private void createUgcEpisodesSectionView(BiliVideoDetail biliVideoDetail) {
            if (seasonsContainer == null || biliVideoDetail == null || biliVideoDetail.mPageList == null) {
                return;
            }
            ...
            com.bilibili.tv.ui.video.widget.PageVideoDetailBinder binder = new com.bilibili.tv.ui.video.widget.PageVideoDetailBinder(false);
            listSection.setData(list, binder);
        ...
        private BiliVideoDetail convertUgcPageToBiliVideoDetail(BiliVideoDetail.Page page, BiliVideoDetail mainVideo) {
            BiliVideoDetail detail = new BiliVideoDetail();
            detail.mAvid = mainVideo.mAvid;
            detail.sourcePage = page;
            detail.mCid = page.mCid;

            String cover = null;
            if (page.firstFrame != null && !page.firstFrame.isEmpty()) {
                cover = page.firstFrame;
            } else if (page.cover != null && !page.cover.isEmpty()) {
                cover = page.cover;
            } else {
                cover = mainVideo.mCover;
            }
            detail.mCover = cover;

            detail.mDuration = page.duration;

            long pubTime = 0;
            if (page.ctime > 0) {
                pubTime = page.ctime;
            } else if (mainVideo.mCreatedTimestamp > 0) {
                pubTime = mainVideo.mCreatedTimestamp;
            }
            detail.mCreatedTimestamp = pubTime;

            detail.mTitle = page.mTitle;
