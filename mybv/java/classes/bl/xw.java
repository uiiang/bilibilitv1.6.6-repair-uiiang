...
public class xw extends xh implements bbb<Message, Boolean>, PlayerMenuRight.a, com.bilibili.tv.ebook.ui.EbookHost {
    ...
    private void S() {
        ...
        // 播放速度优先级：本地记忆（列表 > 视频） > 会话内保持的静态值 > 缓存默认值
        // 播放模式仍采用"会话内保持"策略：
        // 静态字段有效（>=0）时优先使用静态值，保证本次播放内重开菜单圆点保持上次选择；
        // 静态字段失效（新视频构造PlayerMenuRight或离开播放页xl.i()重置）时回退缓存默认值
        int speedId = abd.getSpeedSettingByParams(p(), resolveParams);
        if (speedId < 0) {
            speedId = PlayerMenuRight.speed_id >= 0 ? PlayerMenuRight.speed_id : abd.get_speed_id(p());
        }
